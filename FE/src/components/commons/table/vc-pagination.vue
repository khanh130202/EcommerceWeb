<template>
  <el-row class="vc-pagination">
    <el-col :span="24">
      <el-pagination size="small" background v-model:currentPage="meta.page" v-model:page-size="meta.size"
        :disabled="isReadOnly" :page-sizes="[10, 15, 30, 50, 100]" :total="totalItem" @size-change="onSizeChange"
        @current-change="onPageChange" layout="total, sizes, prev, pager, next, jumper" />
    </el-col>
  </el-row>
</template>

<script setup lang="ts">
import type { MetaResponse } from "@/interfaces/response.interface";
import { toRef, ref, watch, computed } from "vue";

const props = defineProps<{
  pageConfig: MetaResponse;
  isReadOnly?: boolean
}>();

const isReadOnly = toRef(props, "isReadOnly");
const meta = toRef(props, "pageConfig");
const startPage = ref(0);
const endPage = ref(0);
const totalItem = computed(() => {
  return meta.value.total ?? 0;
})
const totalPage = ref(0);

watch(meta, (newVal) => {
  calculate(newVal)
});

watch(props.pageConfig, (newVal) => {
  calculate(newVal)
});

const calculate = (newVal: any) => {
  const page = newVal.page ?? 1;
  const size = newVal.size ?? 10;
  const total = newVal.total ?? 0;
  startPage.value = size * (page - 1) + 1;
  endPage.value = total < size * page ? total : size * page;
  totalPage.value = Math.ceil(total / (size ?? 1));
}

const emit = defineEmits(["changed"]);

const onPageChange = () => {
  emit("changed", meta.value);
};

const onSizeChange = () => {
  meta.value.page = 1;
  emit("changed", meta.value);
};
</script>

<style lang="scss">
.el-select--small .el-select__wrapper {
  width: 90px !important;
}

.el-pagination--small .el-select .el-input {
  width: 120px !important;
}
</style>
