export default {
  ...
  timeStampToString(timeStamp: any) {
    return timeStamp ? this.formatDateTime(new Date(timeStamp * 1000).toDateString()) : ''
  },
  ...
  formatDateTime(input: string) {
    return input ? new Date(input).toISOString().replace(/T/, ' ').replace(/\..+/, '') : null
  },
  ...
  decodeTimestampToDate(timestamp: any) {
    if (typeof timestamp !== 'number' || isNaN(timestamp)) {
      throw new Error('Invalid input. Please provide a valid timestamp as a number.')
    }
